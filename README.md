...
graph TD
    subgraph Closed_Loop_System
        AI2ThorEnv["AI2ThorEnv (Environment)\nOutputs: RGB and Depth Map"]
        YoloDetector["YoloDetector (Perception)\nProcesses images using yolov8n.pt\nOutputs: Bounding Box and Confidence Score"]
        HeuristicBot["HeuristicBot (Decision Agent)\nComputes center offset and depth distance\nOutputs: Action"]
        PipelineRunner["PipelineRunner (Closed-loop Orchestrator)\nCoordinates data flow and saves debug media"]
    end

    AI2ThorEnv -->|1 RGB and Depth Map| PipelineRunner
    PipelineRunner -->|2 RGB Frame and Target Name| YoloDetector
    YoloDetector -->|3 Best Bounding Box and Confidence| PipelineRunner
    PipelineRunner -->|4 Query Next Action| HeuristicBot
    HeuristicBot -->|5 Decides Action| PipelineRunner
    PipelineRunner -->|6 Execute Action Move or Rotate| AI2ThorEnv
```
